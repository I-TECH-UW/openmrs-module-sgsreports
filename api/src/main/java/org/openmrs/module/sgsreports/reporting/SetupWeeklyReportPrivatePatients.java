 ...
 */
public class SetupWeeklyReportPrivatePatients {
	...
	public void setup() throws Exception {
		...
		ReportDesign design = Helper.createRowPerPatientXlsOverviewReportDesign(rd, "weeklyReportPrivatePatients.xls", "weeklyReportPrivatePatients.xls_", null);
		Properties props = new Properties();
		props.put("repeatingSections", "sheet:1,dataset: Weekly Report Private Patients Data Set");
	...
	public void delete() {
		...
		for (ReportDesign rd : rs.getAllReportDesigns(false)) {
			if ("weeklyReportPrivatePatients.xls_".equals(rd.getName())) {
				rs.purgeReportDesign(rd);
			}
		}
		Helper.purgeReportDefinition("Weekly Report Private Patients Report");
	}
	...
	private ReportDefinition createReportDefinition() {
		...
		rd.setName("Weekly Report Private Patients Report");
		...
		// We only wont to see private patients in this report
		PersonAttributeCohortDefinition privatePatients = Cohorts.getPrivatePatients();
		...
		CompositionCohortDefinition privatePatientsWithAnyVisitAtLocation = new CompositionCohortDefinition();
		privatePatientsWithAnyVisitAtLocation.setName("newPediatricClientsFemales");
		privatePatientsWithAnyVisitAtLocation.addParameter(new Parameter("startedOnOrAfter", "Started on or after", Date.class));
		privatePatientsWithAnyVisitAtLocation.addParameter(new Parameter("startedOnOrBefore", "Started on or before", Date.class));
		privatePatientsWithAnyVisitAtLocation.addParameter(getLocationListParameter());
		privatePatientsWithAnyVisitAtLocation.getSearches().put("1", new Mapped<CohortDefinition>(privatePatients, null));
		privatePatientsWithAnyVisitAtLocation.getSearches().put("2", new Mapped<CohortDefinition>(patientsWithAnyVisitsAtLocationBetweenDates, ParameterizableUtil.createParameterMappings("startedOnOrAfter=${startedOnOrAfter},startedOnOrBefore=${startedOnOrBefore},locationList=${locationList}")));
		privatePatientsWithAnyVisitAtLocation.setCompositionString("(1 and 2");
		
		rd.setBaseCohortDefinition(privatePatientsWithAnyVisitAtLocation, ParameterizableUtil.createParameterMappings("startedOnOrAfter=${startedOnOrAfter},startedOnOrBefore=${startedOnOrBefore},locationList=${locationList}"));
	...
	private CohortIndicatorDataSetDefinition createBaseDataSet() {
		...
		dsd.setName("Weekly Report Private Patients Data Set");
