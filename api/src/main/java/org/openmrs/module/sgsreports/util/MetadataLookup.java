...
public class MetadataLookup {
	...
	/**
	 * @return the VisitType that matches the passed uuid, name, or primary key id
	 */
	public static VisitType getVisitType(String lookup) {
		VisitType vt = Context.getVisitService().getVisitTypeByUuid(lookup);
		if (vt == null) {
			try {
				vt = Context.getVisitService().getVisitType(Integer.parseInt(lookup));
			}
			catch (Exception e) {}
		}
		if (vt == null) {
			throw new IllegalArgumentException("Unable to find VisitType using key: " + lookup);
		}
