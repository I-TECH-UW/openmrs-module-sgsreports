...
@Handler(supports = { EncounterAndObsDataSetDefinition22.class }, order = 20)
public class EncounterAndObsDataSetEvaluator22 extends EncounterDataSetEvaluator {
	
	protected static final Log log = LogFactory.getLog(EncounterAndObsDataSetEvaluator22.class);
	 ...
	 * @should evaluate an EncounterAndObsDataSetDefinition2
	 */
	public DataSet evaluate(DataSetDefinition dataSetDefinition, EvaluationContext context) throws EvaluationException {
		
		EncounterAndObsDataSetDefinition22 dsd = (EncounterAndObsDataSetDefinition22) dataSetDefinition;
		
		Object diagnosisParam = context.getParameterValue("diagnosis");
		Object doctorParam = context.getParameterValue("doctor");
		
		if (diagnosisParam == null && doctorParam == null) {
			Mapped<? extends EncounterQuery> filter = getFilter(dsd.getRowFilters(), "defaultFilter");
			dsd.getRowFilters().clear();
			dsd.addRowFilter(filter);
		}
		if (diagnosisParam == null && doctorParam != null) {
			Mapped<? extends EncounterQuery> filter = getFilter(dsd.getRowFilters(), "noDiagnosisFilter");
			dsd.getRowFilters().clear();
			dsd.addRowFilter(filter);
		}
		if (diagnosisParam != null && doctorParam == null) {
