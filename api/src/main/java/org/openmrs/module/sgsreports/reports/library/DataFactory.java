...
public class DataFactory {
	...
	public ScriptedCompositionEncounterDataDefinition getCureSurgicalLogOrTime() {
		...
		EncounterDataDefinition entryIntoOrDefinition = getSingleObsForEncounter(entryIntoOr);
		EncounterDataDefinition entryIntoPacuDefinition = getSingleObsForEncounter(entryIntoPacu);
		
		String script = "import groovy.time.*; if(entryIntoPacu != null && entryIntoOr != null){return TimeCategory.minus(entryIntoPacu.valueDatetime, entryIntoOr.valueDatetime);}";
		
		ScriptedCompositionEncounterDataDefinition timeIntoOr = new ScriptedCompositionEncounterDataDefinition();
		timeIntoOr.setScriptType(new ScriptingLanguage("groovy"));
		timeIntoOr.setScriptCode(script);
		timeIntoOr.getContainedDataDefinitions().put("entryIntoOr", new Mapped<EncounterDataDefinition>(entryIntoOrDefinition, null));
		timeIntoOr.getContainedDataDefinitions().put("entryIntoPacu", new Mapped<EncounterDataDefinition>(entryIntoPacuDefinition, null));
